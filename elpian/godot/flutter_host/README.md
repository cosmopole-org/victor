...
|---|---|---|
| `elpian/widgets` | host → app | `{t, p, c}` serialized widget tree to render |
| `elpian/events`  | app → host | `{cb, args}` — a widget fired; `cb` is the VM-namespaced callback id |
