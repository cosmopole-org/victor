...
| `prelude/caspar.js` | **The Caspar protocol client**: the Caspar-node signed binary action protocol over a `StreamPeerTCP` (framing, dev login, RSA request signing via Godot `Crypto`, creature signalling with correlation-id result routing) plus `CaspiNet`, the CaspiGames service-discovery layer. Composed ahead of a JS guest that has an `import 'caspar.js';` line. Powers the CaspiGames client (which lives in the CaspiGames platform repo) — see [`CASPIGAMES.md`](CASPIGAMES.md). |
