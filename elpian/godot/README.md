...
| `prelude/react.js` | **VReact — React on the VM**: a React-compatible runtime (element factory, the full hook surface, a keyed reconciler mutating retained Godot nodes) whose host config drives VUI. Composed ahead of a JS guest that has an `import 'react.js';` line (which implies `ui.js`). This is what a compiled Next.js-on-Victor app runs on — see `prelude/REACT.md` and `templates/victor-nextjs/`. |
