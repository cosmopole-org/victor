# Elpian ↔ Godot — a tree of Elpian VMs (no JIT) driving the full Godot engine

This directory (`victor/bridge/`) wires the **Elpian VM** — a **cross-language,
universal, no-JIT bytecode VM**, not a Dart VM or a JavaScript VM — into a
**Godot 4 project**, and implements a **C++ controller for the whole engine**
that the VM drives over its `askHost` seam. The VM executes programs written in
**JavaScript *or* a Dart subset** (both compile to the same Elpian AST/bytecode;
see `victor/js2elpian/` and `victor/dart2elpian/`), so everything below works
from either language. A guest program running on the Elpian VM
...
```text
 guest program (JS/Dart)                    C++ GDExtension (native)
 ────────────────────                       ─────────────────────────
...
| `prelude/godot.js` | The **JavaScript** guest library — the JS twin of `godot.dart`: same wire protocol, same `GD`/`GObj`/`VMs`/`GTimer` surface, compiled by `js2elpian`. Composed ahead of a JS guest program. Also ships **`G3`**, a small 3D convenience layer (meshes, materials, lights, camera, and the `SubViewport` 2D↔3D bridge) shared by raw JS guests and the VReact 3D host drivers. |
| `prelude/ui.js` | **VUI — the Victor UI kit**: a full widget toolkit in pure JavaScript built on Godot `Control` nodes over the bridge (see "JavaScript guests & the Victor UI kit" below). Also ships **Flutter-parity** pieces: **`VUI.canvas`** (a `CustomPainter`-equivalent drawing surface over `RenderingServer`, mirroring the `FLCanvas` method surface so one painter serves both the real-Flutter and the native path) and **`VUI.gestures`** (the Flutter event vocabulary — tap/double-tap/long-press/pan/hover/scroll — over `gui_input`), plus `stack`/`positioned`/`align`/`aspectRatio`/`wrap`/`image`. Composed ahead of a JS guest that has an `import 'ui.js';` line. |
| `prelude/caspar.js` | **The Caspar protocol client**: the Caspar-node signed binary action protocol over a `StreamPeerTCP` (framing, dev login, RSA request signing via Godot `Crypto`, creature signalling with correlation-id result routing) plus `CaspiNet`, a super-app service-discovery layer. Composed ahead of a JS guest that has an `import 'caspar.js';` line — see [`prelude/CASPAR.md`](prelude/CASPAR.md). Powers the CaspiGames client, which lives in the CaspiGames platform repo. |
| `prelude/react.js` | **VReact — React on the VM**: a React-compatible runtime (element factory, the full hook surface incl. `useFrame`, a keyed reconciler mutating retained Godot nodes) whose host config drives VUI **and 3D** (Node3D-family host tags + `<scene3d>` over `G3`, so a React tree mixes 2D and 3D freely). Composed ahead of a JS guest that has an `import 'react.js';` line (which implies `ui.js`). This is what a compiled Next.js-on-Victor app runs on — see `prelude/REACT.md` and `templates/victor-nextjs/`. |
| `prelude/flutter.js` | **The Flutter UI bridge (`FL`)**: drives a *real, embedded Flutter engine* as the 2D UI — the twin of `godot.js` but over a declarative widget protocol instead of ClassDB. `FL.mount(node, builder, opts)` spins up a `FlutterView` under a Godot node; the framework re-renders after each event, and widget events route back through the same `__godotDispatch` path and VM sandbox as Godot signals. Composed ahead of a JS guest that has an `import 'flutter.js';` line. See [`FLUTTER.md`](FLUTTER.md), the C++ half in `extension/src/flutter_*`, and the embedded AOT app in `flutter_host/`. |
| `extension/` | The C++ GDExtension: `GodotController` (reflective op interpreter + Variant↔JSON marshaling + handle table), `ElpianCallable` (Dart-closure-backed `Callable`), `ElpianVM` (the scene node), `FlutterController` + `FlutterView` (the embedded-Flutter UI bridge, gated behind `ELPIAN_WITH_FLUTTER`), build files (CMake + SCons). |
...
* **Spawning** — `VMs.spawn(source, node, {label, limits, permissions,
  maxHostCalls, maxBytesMoved, scene})` compiles a new guest program into a
...
```sh
# 1. The Rust half (from victor/):
cargo build -p elpian-godot-capi --release

# 2. The C++ half (from victor/bridge/extension/):
...
Both trigger on pushes to `main`/`master` touching `victor/**` and support
